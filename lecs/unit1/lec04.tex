...
\vspace{25pt}
\subtitle{Unit 1: Introduction to probability and statistics \\ Lecture 4: Moments, characteritics function, known probability distributions, bivariate probability distributions}
...
\begin{frame}{Expected value}
    The measures or descriptors of a random variable determine important features of their behaviour and thus describe the \emph{pmfs} and \emph{pdfs}. The descriptors are defined upon the \emph{pmfs} o \emph{pdfs} and are known as the \emph{statistics} of the random variable. 
    \begin{block}{Expected value}
        ...
        \]
        If $X$ is continuos, the \alert{expected value} is:
        \[
...
The \emph{moment of order $m$} can be obtained by deriving $M_X (t)$ with respect to $t$ and evaluating the derivative for $t=0$, so:
\[
...
\]
where the \alert{cummulants},  $\kappa_n = \frac{1}{i^n} \frac{d^n K_Z (t)}{dt^n} \vert_{t=0}$. The \emph{cumulants} are related to the pdf  moments and viseversa so:
\begin{align*}
    \kappa_1 &= \mu_1 \\
...
\begin{frame}{Estimation of parameters}
    ...
    \begin{block}{Method of probability weighted and L-moments}
        ...
        For a random variable $X$ with \emph{cdf} $F_X (x)$, the \alert{probabilistic weighted moments (pwms)}, which are a class of \emph{L stimators} (linear functions of an ordered sample), are defined as:
        \[
            M_{ijk} = E \left[ X^i \{F_X (x)\}^j \{1 - F_X (x) \}^k \right] = \int_0^1 {x(F)}^i F^j (1 - F)^k dF
        \]
        where $x(F)$ is the quantile or inverse \emph{cdf} function of $X$, and $j$ and $k$ take values as  $0, 1, \cdots, m$, where $m$ is the number of sample \emph{pwms}. For $j=k=0$ this equation provides the moment of order $i$ about zero. In the application of \emph{pwms}, it is often convenient to $i=1$, and $j=0$ or $k=0$. Then the first $m$ sample \emph{pwms} (that is, for $j = 0, 1, \cdots, m$, or $k = 0, 1, \cdots, m$ evaluated by associating empirical probabilities with each item of data in a sample), are equated to the first $m$ population \emph{pwms} obtained from the equation for $M_{ijk}$.
    \end{block}
        \vspace{-6pt}
    ...
    \[
            \ln y = -\exp\left( - \frac{x-b}{a}\right) \quad \quad \ln( -\ln y) = -\frac{x-b}{a} \quad \quad x = b - a\ln( -\ln y)
        \]
        \vspace{-5pt}
...
\]
where $p_i$ is the  probability of nonexceedance at which the $x_i$ should be plotted after ranking in ascending order. Accordingly:
...
Suppose that we have the following table with the annual maximum precipitation depth ($x_i$ (mm)) for a 3-h storm duration for a period of time. The table shows the ordered precipitation and the probabilities of each value $p_i$. 

\begin{minipage}{0.50\textwidth}
\centering
    \includegraphics[width=0.98\textwidth]{ta321.png}
\end{minipage}
 \hfill
\begin{minipage}{0.48\textwidth}
...
\begin{frame}{Estimation of parameters}
    \begin{block}{Maximum likelihood method}
        For a given random variable $X$, a sample of size $n$ is taken from the  sample space, and the \emph{pdf} is $f_X (x)$. The \alert{likelihood function} of $\theta$, where $\theta$ is vector of $m$ parameters, is represented as:
        \[
...
\begin{frame}{Estimation of parameters}
    \begin{exampleblock}{Flood exeedance}
        ...
        The derivative is:
        \[
            \frac{d \left( \ln L (p) \right)}{dp} = \frac{1}{p} \sum_{j=1}^n x_j - \frac{1}{1-p}\sum_{j=1}^n (1-x_j) = \frac{\sum_{j=1}^n x_j}{p} - \frac{n - \sum_{j=1}^n x_j}{1-p}
        \]
    ...
    \end{exampleblock}
    The \emph{maximum likelihood method} is the most implemented method. However, large samples are required to prevent the estimator becomes biased. This estimator does not have a low variance in comparison with others. Sometimes the estimators cannot be obtained analytically so numerical methods are needed.
\end{frame}
...
\begin{frame}{Joint probability distribution of continuous variables}
    ...
    \begin{block}{Joint pdf and cdf for continuous $X$ and $Y$ variables}
        These concepts can be applied to $n$ random variables $X$ defined on the same probability space. If $X_1, X_2, \cdots, X_n$ are variable defined on the same probability space, $(X_1, X_2, \cdots, X_n)$ is an \alert{n-dimensional continuous random variable} if and only if there exists a function $f_{X_1, X_2, \cdots, X_n} (x_1, x_2, \cdots, x_n) \geq 0$. Accordingly:
        \begin{align*}
            \displaystyle
            &F_{X_1, X_2, \cdots, X_n} (x_1, x_2, \cdots, x_n) \\
            &\equiv Pr[( X_1 \leq x_1) \cap ( X_2 \leq x_2) \cap \cdots \cap ( X_n \leq x_n)] \\
...
\begin{frame}{Joint probability distribution of continuous variables}
    ...
    \begin{exampleblock}{Storm intensity and duration}
        For storms occurring in place on earth, it is characterized their \emph{intensity} and  \emph{duration}, where the intensity is the average amount of rain fell. Suppose that intensity and duration are two continuous random variables, $Y$ and $X$, respectively. It is known that the \emph{cdf} of $X$ and $Y$ are represented by:
        \[
        ...
        \vspace{-5pt}
        \begin{align*}
            -e^{-by}+e^{-ax-by-cxy} \leq 0 \quad\quad e^{-ax-by-cxy} \leq e^{-by} \quad\quad -x(a+cy) \leq 0
        \end{align*}
...
\]
        ...
        \end{align*}
        Accordingly, the for $x=y=0$, $f_{X,Y}(0,0)= ab-c$. Since the joint \emph{pdf} is a non-negative function, the inequality $ab-c \geq 0$ must hold, so the upper bound of $c$ is $c\leq ab$. Therefore, the bivariate exponential distribution is defined for $0 \leq c \leq ab$. 
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Joint probability distribution of continuous variables}
    \begin{exampleblock}{Storm intensity and duration}
        The conditional cdf is (making $y \equiv u$):
         \begin{align*}
             \displaystyle
             F_{Y|X} (x,y) &= \int_0^y a^{-1}\left[ (a+cu)(b+cx)-c \right]e^{-u(b+cx)} du\\
...
\begin{frame}{Joint probability distribution of continuous variables}
    ...
    \begin{block}{Marginal probability density function}
        ...
        \[
            \displaystyle
            f_Y (y) = \int_{-\infty}^{\infty} f_{Y|X} (x,y) f_X (x) dx = \int_{-\infty}^{\infty} f_{X,Y} (x,y) dx
        \]
    \end{block}
\end{frame}
...
\begin{frame}{Properties of multiple variables}
    ...
    \begin{block}{Covariance and correlation}
        The \emph{expectation operator (E)} introduced before for a single variable can be used on two or more random variables. For instance, for the linear combination of two random variables $X_1$ and $X_2$, $E[a X_1 + b X_2 ] = aE[X_1 ]+bE[X_2 ]$, where $a$ and $b$ are constants. Accordingly, the variance of $aX_1 + bX_2$ is:
        \[
        ...
        where $ E[X_1 X_2] = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} x_1 x_2 f_{X_1 , X_2} (x_1, x_2) d_{x_1} d_{x_2}$. If $X_1$ and $X_2$ are independent:
         \begin{align*}
         E[X_1 X_2] &= \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} x_1 f_{X_1}(x_1) x_2 f_{X_2}(x_2)  d_{x_1} d_{x_2} \\
...
\begin{frame}{Properties of multiple variables}
    \begin{exampleblock}{Storm intensity and duration}
        ...
        This confirm that there is no covariance between $X$ and $Y$ if they are \emph{independent}.
    \end{exampleblock}
    \begin{block}{Coefficient of linear correlation}
        Note that $Cov[X_1,X_2]$ is positive and large when both variables are large or small with respect to their means. In contrast,  $Cov[X_1,X_2]$ can be negative and large when both variables are equidistant (e.g. one small and the other large). Note that covariance is a measure of the \emph{linear relationship} between variables; no linearly related variables yield no covariance. The linear relationship between $X_1$ and $X_2$ is defined as the \alert{coefficient of linear correlation ($\rho$)}:
    \[
...
\begin{frame}{Properties of multiple variables}
    ...
    \]
    This means that if the joint \emph{mgf} of two variables equal the product of individual \emph{mgf}s, the two variables are \emph{statistically independent}. 
        \end{block}
...
\begin{frame}{Probability distributions}
    \begin{block}{Generalities}
        \begin{itemize}
            ...
            \item Probability distributions, according to the random variable type, can be:
                \begin{itemize}
...
\begin{frame}{Discrete probability distributions}
    \begin{block}{The \alert{Bernoulli} distribution}
        Suppose that one is analysing an experiment where the outcomes are either \emph{success} or \emph{failure}. The two events are \emph{mutually exclusive} and \emph{collective exhaustive}. Experiment examples are whether rain or not in a given day. Suppose that $X$ is a random variable that can take values of 1 (\emph{success}) or 0 (\emph{failure}). Accordingly, the \emph{pmf} of $X$ is
\[
p_X (X = x) = 
\left\{
\begin{array}{l}
...
\begin{align*}
    E[X] &= \sum_x^{[0,1]} x p_X = 1xp + 0(1-p) = p \\
...
\begin{frame}{Discrete probability distributions}
    ...
    \begin{block}{The \alert{binomial} distribution}
        ...
        \]
        where $n$ and $p$ are parameters of the distribution, $x=0,1,\cdots, n$, $0 \leq p \leq 1$ and $\binom{n}{x}=\frac{n!}{x! (n-x)!}$; this term represents the possible combinations to obtain $x$ success from $n$. The \emph{cmf} is:
        \[
...
\begin{exampleblock}{Flooding of a road} % From Kotte
        The probability that a road is flooded once in a given year is $p=0.1$. Compute the probability that the road will be flooded at least once during a five-year period. So the $Pr[X \geq 1] = Pr[X = 1;5,0.1] + Pr[X=2;5,0.1] + Pr[X=3;5,0.1]+ Pr[X=4;5,0.1] + Pr[X=5;5,0.1] = 0.3281 + 0.0729 + 0.0081 + 0.0005 + 0.00001 = 0.4095$. An alternative to  estimate this probability is to determine the probability of no flooded during the period ($Pr[X < 1]= 1-Pr[X \geq 1]$). So $Pr[X \geq 1] = 1-Pr[X=0;5,0.1] = 1-0.5905 = 0.4095$. Note that in reality, the values of $p$ can change year to year so the application of this model can be inaccurate. 
    \end{exampleblock}
    \end{frame}

\begin{frame}{Discrete probability distributions}
    \begin{block}{The \alert{geometric} distribution}
        This distribution characterizes the probability of a random variable $X$ that represents the \emph{Bernoulli's trials} upon one get a success. So the \emph{pmf} is:
        \[
            p_X (x) = Pr[X = x; p] = (1-p)^{x-1} p
        \]
        and the \emph{cdf} is:
        \begin{align*}
            F_X (x) = Pr[X\leq x] &= \sum_{j=1}^x p_X (j) = \sum_{j=1}^x (1-p)^{j-1} p = p \sum_{j=1}^x (1-p)^{j-1} = p\left(\frac{1-(1-p)^x}{1-(1-p)} \right) \\
                                  &= 1-(1-p)^x
                                  \end{align*}
        where $p$ ($0 < p \leq 1$) is the probability of success in the Bernoulli's trials, and $X = 0,1,2, \cdots$, $x-1$ failures before the first success. Note that the term $\sum_{j=1}^x (1-p)^j$ is  a \emph{finite geometric series} that converge to $\left(\frac{1-(1-p)^x}{1-(1-p)} \right)$. 

        To obtain the mean,  $E[X] = \sum_{x=1}^\infty x (1-p)^{x-1} p = p \sum_{x=1}^\infty x (1-p)^{x-1} = \frac{1}{p}$. Note that according to the definition of a \emph{geometric series} $\sum_{x=1}^{\infty} (1-p)^x$, this converge to $\frac{1}{p}-1$. So $E[X]$ is obtained from deriving both sides of the geometric series. This value means that the average number of trials to the first occurrence is the reciprocal of the probability of occurrence on any trial. For instance is the probability that a rainfall overcome a threshold is $p$, the average number number to get the first maximum rainfall is $1/p$. 
...
\begin{frame}{Discrete probability distributions}
    \begin{block}{The \alert{Poisson} distribution}
        Suppose that the domain in which a trial occurs (e.g. time interval or delimiting space) is very short. This means that the number of trials can be many and when the domain get smaller the number of trial tend to $\infty$. Support that an engineer is interested to know the number of cars that cross an intersection during a defined interval of time $t$ in seconds. Suppose that $t=1s$, so the probability that a car cross during $t$ is p. For 2 or more cars, the probability $p$ is very low. So the total number of vehicles $X$ in $n=t$ independent trials follow a \emph{Binomial distribution $B(n,p)$} $p_X (x) = B(x;n,p) = \binom{n}{x} p^x (1-p)^{n-x}$

        Suppose that the interval $t$ get quite small, so the \emph{number of trials (n)} and $p$ (success probability) decrease. This means that the expected number of events in the interval $\nu$ remains constant and equal to $np$. Thus the \emph{pmf} of $X$ in the limit  as $t\rightarrow 0$,  $n\rightarrow \infty$ and $p\rightarrow 0$ is
        \begin{align*}
            p_X (x) &= \frac{n!}{x! (n-x)!} \left(\frac{\nu}{n}\right)^x \left(1-\frac{\nu}{n}\right)^{n-x}\\
                    &= \frac{\nu^x}{x!} \left(1-\frac{\nu}{n}\right)^{n}\frac{n!}{n^x (n-x)!} \frac{1}{\left(1-\nu/n\right)^x} \\
                    &= \frac{\nu^x}{x!} \left(1-\frac{\nu}{n}\right)^n \left[ \frac{n (n-1) (n-2) \cdots (n-x+1)}{\left( n(1-\nu/n) \right)^x } \right] 
        \end{align*}
...
\begin{frame}{Discrete probability distributions}
    ...
    \end{block}
\begin{minipage}{0.49\textwidth}
    Note that for large $n$ $n (n-1) (n-2) \cdots (n-x+1) \approx n^x$ and $\left( n(1-\nu/n) \right)^x  \approx n^x$. So 
        \[
            p_X (x) = \frac{\nu^x}{x!} \left(1-\frac{\nu}{n}\right)^n 
        \]
         is known as the \alert{Poisson distribution} with parameter $\nu$.
        
        The $\mu_X = E[X] = \nu$ and the $\sigma_X^2 = Var[X] = E[X^2] - (E[x])^2 = \nu$ 

 \end{minipage}
\hfill
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=0.9\linewidth]{fiV321.jpeg} 
\end{minipage}

\end{frame}

 \begin{frame}{Discrete probability distributions}
    \begin{block}{The \alert{Poisson} process}
    \end{block}
\begin{minipage}{0.49\textwidth}
         If one is interested in a larger time interval, let's say, $2t$, the number of trials at any stage in the limit would be twice, and the parameter of the resulting \emph{Poisson distribution} is $2\nu$. In general, $\nu$ would be equal to $\lambda t$, so the \emph{pmf} is:
        \[
            p_X (x) = \frac{(\lambda t)^x e^{-\lambda t}}{x!} \quad \quad x= 0,1,2, \cdots , \infty
        \]
...
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{fiV322.jpeg} 
\end{minipage}

This \emph{pmf} is known as the \alert{Poisson process} and it is a \emph{stochastic process}. The \emph{Poisson process} is a random function of time (usually). In hydrology, we are interested in a random function $X(t)$  whose values at any given time $t$ is the \emph{random number} of arrivals which has occurred since $t=0$. Note that while samples of a random variable $X$ are the numbers $x_1, x_2, \cdots$, observations of a \emph{random process} $X(t)$ are sample functions of time $x_1(t), x_2 (t), \cdots$. Some examples of a \emph{Poisson process}:
\begin{itemize}
    \item The number of vehicule that arrive to a certain point vs time.
    \item Total accumulated rainfall vs time
\end{itemize}
Note that for any fixed value of $t=t_0$, the value $X(t_0)$ of a stochastic process is a random variable with an appropriate distribution $p_{X(t_0)} (x)$ with $\mu_{X(t_0)}$ and $\sigma_{X(t_0)}$. We can say that the \emph{pmf} of $\mu_X$ and $\sigma_X$ are functions of time for a $X(t)$ \emph{random function}. Note that \emph{pmf} of the random variable $X(t_0)$ is the \emph{Poisson distribution} with parameter $\nu= \lambda t_0$.
\end{frame}

 \begin{frame}{Discrete probability distributions}
    \begin{block}{The \alert{Poisson} process}
    \end{block}
    To be a \emph{Poisson process} the underlying physical mechanism generating the \emph{arrivals} or \emph{incidents} most satisfy the following assumptions:
    \begin{itemize}
        \item \alert{Stationarity}: The probability of an \emph{arrival} or \emph{incident} in a short interval of time $[t, t + h]$ is approximately $\lambda h$ for any $t$.
        \item \alert{Non multiplicity}: The probability of two or more events in a short interval of time is neglilable compared to $\lambda h$.
    \item \alert{Independence}: The number of \emph{incidents} or \emph{arrivals} in any interval of time is  independent of the number in any other (non-overlapping) interval of time.
    \end{itemize}
    Shortly, if we are observing a \emph{Poisson process},  $X(t)$ at any $t$ follows a \emph{Poisson distribution} with $\mu_{X(t)} = \lambda t$. So $\lambda$ is  usually referred to as the \emph{average rate of arrivals} of the \emph{Poisson process}. Regarding that \emph{incidents} or \emph{arrivals} occur independently along time with a constant rate of occurrence, a \emph{Poisson process} is often referred as the model of random events. Therefore, it has been employed to describe the occurrence of storms, floods, etc. 
\end{frame}
 ...
 \begin{frame}{Discrete probability distributions}
    ...
    \begin{exampleblock}{Atmospheric pollution}
Dust particles in the atmosphere in a certain location can cause health problems. The count of dust particles for 100 air samples was made and register in the following table.
    ...
    \begin{tabular}{lccccccc}
        & \multicolumn{6}{c}{\# of dust particles per unit volume of air} & \\
        \cline{2-7}
        & 0 &  1 &  2 & 3 & 4 & $>$ 4 & $\sum$\\
        \cline{2-8}
        Obs. frequency & 13 & 24 & 30 & 18 & 7 & 8 & 100 \\
        Poisson. frequency & 12 & 26 & 27 & 19 & 10 & 6 & 100 \\
    \end{tabular}

    Using the concept of \emph{expected value} for a \emph{discrete random variable}, the estimated \emph{mean} of dust particle is $\bar{X} = E[X] = \sum_{i=1}^n x_i p_X (x_i) = 2.14$ particles. If we assume that the occurence of dust particles in the air sample follows a \emph{Poisson distribution} where $E[X] = \nu$ the theorethical frequency of occurrence can be obtained using
    \[
        p_X (X = x | \nu = 2.14) = \frac{2.14^x e^{-2.14}}{x!} \quad \text{ for} x=0,1,2,3,4,  >4
    \]
        ...
        It is the simplest distribution, and as it is indicated by its name, the \emph{pdf} is constant over a defined interval $a \leq x \leq b$. The \emph{pdf} is:
         \begin{align*}
             ...
             f_X (x) &= \frac{1}{b-a} \text{, for } a\leq x \leq b \\
                     &= 0 \text{, otherwise}
        \end{align*}
        This distribution is also known as the \emph{rectangular distribution}. Note that all values between $a$ and $b$ are equally probable and the area under the \emph{pdf} is equal to 1. The probability that $X$ fall in $(c,d)$ is $Pr[c < X < d] = \frac{d-c}{b-a}$. The \emph{unit uniform distribution} is obtained when $a=0$ and $b=1$ ans it is commonly used to generate random variables in simulations. 
    \end{block}
    \vspace{-3pt}
\begin{minipage}{0.49\textwidth}
\centering
...
\begin{frame}{Continuous probability distributions}
    \begin{block}{\alert{Exponential} distribution}
        From the \alert{Poisson process} discussed previously, if we denote by $T$ a random variable that indicate the time to the first arrival, then the probability that $T$ exceeds some value $t$ is equal to the probability that no events occur in that time interval of length $t$. While former probability is 1 - $F_T (t)$, the latter probability $p_X (0)$ is zero, which is the probability of a Poisson random variable $X$ with parameter $\lambda t$. Replacing:
        \[
...
\]
    ...
    \end{block}
     ...
     \begin{exampleblock}{Floods affecting construction}
         Engineers are concerned with the occurrence of a flood exceeding 100 m$^3$ s$^{-1}$ that can seriously affect the construction of a bridge. If this streamflow is exceeded once every five years on average based on historical flow records, what is the change that the bridge construction, which is scheduled to last 14 months, can proceed without interruptions or detrimental effects? 
...
f_X (x) = 
\begin{cases}
    \frac{\lambda (\lambda x)^{k-1} e^{-\lambda x}}{\Gamma (k)}& \text{for } x \geq 0, k>0, \lambda >0 \\
    0 & \text{otherwise }
\end{cases}
\]
Note that $\Gamma (k)$ arise here as constant to normalize the function. $k$ is denominated as the \emph{shape parameter} as it defines the shape of the \emph{pdf} and $\lambda$ the \emph{scaling parameter} (see the figures).
...
\begin{frame}{Continuous probability distributions} 
 \begin{exampleblock}{Maximum flows}
     Based on a histogram of data of maximum annual river flows between 1930 and 1960, it was assume that the data followed a \emph{gamma pdf}. The parameters were estimated and equal to $k= 1.727$ and $\lambda = 0.00672$ cfs$^{-1}$. According to these values, $\mu_X = \frac{k}{\lambda} = \frac{1.727}{0.00672} = 256.7$ cfs, $\sigma_X  = \frac{\sqrt{k}}{\lambda} = 190$ cfs. Also, the probability that the maximum flow is less than 400 cfs in any year is $F_X (400) = \frac{\Gamma (1.727, \lambda 400)}{\Gamma (1.727)} = \frac{0.71}{0.914} = 0.78$. Note that the values to estimate the numerator and denominator are taken from tables of the gamma function. 
    \end{exampleblock}
\end{frame}

\begin{frame}{Continuous probability distributions} 
    \begin{block}{The \alert{Beta} distribution}
        The \alert{Beta distribution} is used to model a \emph{random variable} $X$ whose values go from  0 to 1, and thus it is important in decision methods. The \emph{beta pdf}  is given by: 
\[
f_X (x) =
\begin{cases}
...
\end{cases}
\]
where $\alpha$ and $\beta$ are parameters that $\alpha > 0$ and $\beta > 0$, and the \emph{beta function} is:
\[
...
F_X (x) =
\begin{cases}
    1- e^{-\left( \frac{x}{\lambda} \right)^{\beta}}, & \text{for } x > 0  \\
    0 & \text{otherwise }
\end{cases}
\]
...
\end{minipage}
Note that from the definition of the \emph{pdf} and \emph{cdf}, if a random variable $X \sim \text{ Weibull } (\beta, \lambda)$, then $Y = \left( X/\lambda \right)^{\beta} \sim \text{ exponential } (\lambda=1)$, $f_Y (y) = e^{-y}$. 
\end{frame}
...
\begin{minipage}{0.59\textwidth}
     The approximate lower 99\% confidence limit for $\beta$ is $\frac{\hat{\beta}}{2n} x_{2n,0.99}^2 = \frac{2.59}{20} x 8.26 = 1.07$. Also, quantile estimates can be obtained using the estimates of the Weibull parameters and the preceding equations. For example the annual minimum 10-day flow with a return period of 10 yrs is: 
\vspace{-5pt}
     \[
...
\begin{frame}{Continuous probability distributions} 
    \begin{block}{The \alert{Lognormal} distribution}
        We know that the addition of large number of small random effects will tend to make the distribution of the aggregate approach normal. In contrast, if the phenomena arises from the multiplicative effect of a large number of uncorrelated factors, the distribution tend to be \alert{Lognormal}, that is, the logarithm of a random variable $X$ becomes normally distributed. Phenomena such as the particle size of sediment samples are \emph{Lognormal}, and the interarrival times of earthquakes. As this distribution is defined for non-negative values is used to describe daily average streamflows, maximum streamflows, and daily, monthly and annual rainfall. If $X$ is a positive random variable, then $Y = \ln (X)$. Accordingly, if $Y$ has $N \sim (\mu_Y , \sigma_{Y^2})$, $X$ has a \emph{Lognormal distribution} $LN \sim (\mu_{\ln(X)}, \sigma_{\ln(X)}^2)$. So the \emph{pdf} is:
        \[
...
\hfill
\begin{minipage}{0.59\textwidth}
    By using the \emph{moment-generating function}, the expectation operator can be written as: 
    \begin{align*}
