...
\vspace{25pt}
\subtitle{Unit 1: Introduction to probability and statistics \\ Lecture 2: Descriptive statistics, univariate and bivariate statistics}
...
\begin{frame}{Definitions}
    ...
    Data is analysed on \alert{data sample}, so that, \emph{statistical and probabilistic analysis} of samples serve to make \alert{inferences} on the data population.
\end{frame}
\begin{frame}{Hydrologic data}
    \begin{block}{Data types}
        ...
        \begin{enumerate}
            ...
            \item \alert{Laboratory or field experiment data}: Data acquired controlling external factors. Used in basic and applied research. 
...
\begin{frame}{Graphical representation}
    ...
    \begin{minipage}[]{0.68\textwidth}{\alert{Bar chart}}
        \vspace{0pt}
...
Continuous data can be represented on a single axis when the data set is small. 
    ...
    %\includegraphics[width=0.9\textwidth]{fi112.png}
...
\begin{frame}{Graphical representation: Histogram}
        ...
        \begin{enumerate}
            \item Sort the data in ascending order $x_{min}\cdots x_{max}$.
            \item Estimate the data range ($r$) as $r = x_{max} - x_{min} $.
            \item The data is initially divided  into groups or classes ($n_c$) according to the data magnitude where each group has a lower and upper limit.  According to Sturges (1926), $n_c = 1 + 3.3 \log_{10} n $, donde $n$ is the number of data. Note that $n_c$ must be an positive integer and is recommended that $5 \le n_c \le 25$. $n_c$ can be estimated using Freedman and Diaconis (1981) formula  $n_c = \frac{r n^{1/3}}{2\left(Q_3 - Q_1\right)}$, where $Q_3$ is the median of the upper half of the data and $Q_1$ is the median of the lower half of the data. The difference $Q_3 - Q_1$ is known as the \emph{interquartile range}.
...
\begin{frame}{Measures of central tendency}
    ...
    \centering
    \includegraphics[width=0.88\textwidth]{fie3.JPG}
\end{frame}
...
\begin{frame}{Measures of dispersion}
    ...
    \begin{block}{Sample variance and standard deviation}
        ...
        \captionof{figure}{Histogram of a data sample with the same $\bar{x}$ and different variance.}
    \end{block}
\end{frame}
...
\begin{frame}{Measure of asymmetry}
    ...
    \begin{block}{sample coefficient of skewness}
        ...
        \captionof{figure}{Histogram of a data sample with the same $s^2$ and different skewness.}
        \end{center}
        \vspace{-0.3cm}
...
\begin{frame}{Measure of asymmetry}
    ...
    \begin{center}
        ...
        \end{center}
        According to this, the \alert{ quartile coefficient of asymmetry} is:
    $$
...
The measure of the relative ascending steepness nearby the mode in the histogram or in the frequency curve is said to be a measure of the peakedness.
    \begin{block}{Sample coefficient of kurtosis}
        ...
        \captionof{figure}{Histogram of a data sample with negative and positive kurtosis.}
    \end{block}
\end{frame}
...
\begin{frame}{Box plot}
    ...
    \vspace{-0.2cm}
        ...
        \captionof{figure}{Box plot structure.}
...
\begin{frame}{Sample covariance and correlation coefficients}
        ...
        \captionof{figure}{Scatter plots and $r_{X,Y}$.}
\end{frame}
...
\begin{frame}{Q-Q plots}
    ...
    \includegraphics[width=0.70\textwidth]{fi143.png}
        \vspace{-0.1cm}
\end{frame}
