...
%-------
% From Bierkens
\section{Estimating the local conditional distribution}
\begin{frame}{Estimating the local conditional distribution}
    \begin{block}{Generalities}
    \end{block}
        \begin{itemize}
            \item \emph{Kriging} can also be used to estimate for each \emph{non-observed location} the \emph{probability distribution} $f_z(z; \mathbf{x} | z(\mathbf{x}_i), i= 1, .., n)$, i.e the \emph{probability distribution} given the \emph{observed values} at the \emph{observation locations}.
            \item Recall that each \emph{realisation} is \emph{conditioned} by the \emph{observations}, i.e. it passes through the observed value, but is free to vary between observations. The farther away from an observation, the more the realisations differ.
            \item This is reflected by the \emph{conditional pdf} $f_z(z; \mathbf{x} | z(\mathbf{x}_i), i= 1, .., n)$ at a given location. The farther away from an observation, the larger the \emph{variance} of the \emph{conditional pdf}, which means the more \emph{uncertain} we are about the actual but unknown value $z(\mathbf{x})$.
            \item The following  methods can be used to estimate the \emph{conditional pdf} $f_z(z; \mathbf{x} | z(\mathbf{x}_i), i= 1, .., n)$ through kriging.
        \end{itemize}
\end{frame}

\begin{frame}{Estimating the local conditional distribution}
    \begin{block}{Multivariate Gaussian random functions}
    \end{block}
    If, apart from being \emph{wide sense stationary}, the \emph{random space function} is also \emph{multivariate Gaussian distributed} then we have:
    \begin{enumerate}
        \item The \emph{kriging error} is \emph{Gaussian distributed} with \emph{mean} zero and \emph{variance} equal to the \emph{simple kriging variance} $\sigma_{SK}^2 (\mathbf{x}_0) = var\left[\hat{Z}(\mathbf{x}_0) - Z(\mathbf{x})\right]$.  A \emph{95\%-prediction interval} would then be given by $\left[ \hat{z}_{SK}(\mathbf{x}_0) - 2\sigma_{SK}(\mathbf{x}_0), \hat{z}_{SK}(\mathbf{x}_0)+ 2\sigma_{SK}(\mathbf{x}_0) \right]$ where $\hat{z}_{SK}(\mathbf{x}_0)$ is the \emph{simple kriging prediction}.
        \item The \emph{conditional cumulative probability distribution function (ccpdf)} is \emph{Gaussian} with \emph{mean} equal to the \emph{simple kriging prediction} $\hat{z}_{SK}(\mathbf{x}_0)$ and \emph{variance} equal to the \emph{variance of the simple kriging prediction error} $\sigma_{SK}^2(\mathbf{x}_0)$:
  \begin{equation}
                    F_{z|z_1,\cdots,z_n} (z; \mathbf{x}_0 | z(\mathbf{x}_i), i=1,\cdots,n) = \frac{1}{\sqrt{2\pi \sigma_{SK}^2 (\mathbf{x}_0)}} \int_{-\infty}^z e^{\frac{-\left[z-\hat{y}_{SK}(\mathbf{x}_0) \right]^2}{\sigma_{SK}^2 (\mathbf{x}_0)}} dz
                \label{e731}
            \end{equation}
            where $z(\mathbf{x}_1),\cdots,z(\mathbf{x}_n)$ are the observed values at locations $\mathbf{x}_1 ,\cdots,\mathbf{x}_n$ respectively.
           
    \end{enumerate}
    The second property is very convenient and the reason why the \emph{multivariate Gaussian} and \emph{stationary random spatial function} is very popular model in \emph{geostatistics}.
\end{frame}
...
\begin{frame}{Estimating the local conditional distribution}
    ...
    \end{block}
    \begin{itemize}
        \item After performing \emph{simple kriging predictions}, one is able to give an estimate of the \emph{ccpdf} of $Z(\mathbf{x})$ \emph{exceeding} a given \emph{threshold} for every location in the domain of interest.
        \item For instance, if $Z(\mathbf{x})$ is a \emph{concentration of some pollutant} in the groundwater and $z_c$ is \emph{critical threshold} above which the \emph{pollutant} becomes a \emph{health hazard}, \emph{simple kriging} and equation~\ref{e731} can be used to map the \emph{probability of exceeding} this \emph{threshold}, given the concentrations found at the observation locations.
        \item Instead of \emph{delineating} a \emph{single plume} based upon some \emph{predicted value}, \emph{several alternative plumes} can be delineated, depending on which \emph{probability contour} is taken as its boundary.
...
\begin{frame}{Estimating the local conditional distribution}
     \begin{block}{Log-normal kriging}
    \end{block}
    \begin{itemize}
        ...
        \item A frequently used \emph{random space function} model to describe these variables is the \emph{multivariate log-normal distribution}.
        \item If $Z(\mathbf{x})$ is \emph{multivariate log-normal distributed}, the natural logarithm $Y(\mathbf{x})=\ln(Z(\mathbf{x}))$ is \emph{multivariate normal distributed}. \alert{Log-normal kriging} then consists of the following steps:
            \begin{enumerate}
                ...
                \item Estimate the \emph{semivariogram} $\hat{\gamma}_Y (\mathbf{h})$ from the log-transformed data $y(\mathbf{x}_i)$ and fit a permissible model (note: that mean $m_Y$ must be determined and assumed known if \emph{simple kriging} is used).                 
                \item Using the \emph{semivariogram} $\hat{\gamma}_Y (\mathbf{h})$, the data $y(\mathbf{x}_i)$ (and the mean $m_Y$ in case of \emph{simple kriging}), the kriging equations are solved to obtain at every non-observed location $\mathbf{x}_0$ the \emph{prediction} $\hat{Y}_{SK} (\mathbf{x}_0)$ and \emph{prediction error variance} $\sigma_{YSK}^2 (\mathbf{x}_0)$ in cased of \emph{simple kriging} or $\hat{Y}_{OK} (\mathbf{x}_0)$, $\sigma_{YOK}^2 (\mathbf{x}_0)$ in case of \emph{ordinary kriging}.
...
\begin{enumerate}
    ...
    \item If $Y(\mathbf{x})$ is \emph{multivariate Gaussian} distributed and \emph{stationary}, the \emph{conditional cumulative probability distribution function (ccpdf)} can be calculated from the \emph{simple kriging} prediction $\hat{y}_{SK}(\mathbf{x}_0)$ and prediction \emph{variance} as:
            \begin{equation}
...
\begin{frame}{Estimating the local conditional distribution}
     \begin{block}{Kriging normal-score transforms}
    \end{block}
    ...
    \begin{enumerate} 
        ...
        \item The \emph{cumulative probability} associated with observation $z(\mathbf{x}_k) = z_k$ with rank $r$ is estimated as:
            \begin{equation}
...
\begin{frame}{Estimating the local conditional distribution}
    ...
    \end{block}
            \vspace{-5pt}
            If we assume that the \emph{normal-score transforms} are \emph{stationary} and \emph{multivariate Gaussian distributed},  the \emph{local ccpdfs} can be obtained through \emph{simple kriging} as follows:
    \begin{enumerate}
        \item Perform a \emph{normal-score transform} of the observations such as described above.
        ...
        \item The local \emph{ccpdf} is then given by
            \vspace{-5pt}
            \begin{equation}
                \begin{aligned}
...
\begin{frame}{Estimating the local conditional distribution}
     \begin{block}{Kriging normal-score transforms}
    \end{block}
    The figure below shows the \emph{histogram} and the \emph{semivariogram} of the \emph{normal score transforms}. It can be seen that \emph{semivariogram} is less noisy than that of the \emph{non-transformed data}, because transformation decreases the effect of the very large values.
        \begin{center}
...
%-------
% From Bierkens
\section{Geostatistical simulation}
