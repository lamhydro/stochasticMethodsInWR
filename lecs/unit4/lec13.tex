...
\vspace{25pt}
\subtitle{Unit 4: Time series analysis \\ Lecture 11: Univariate, linear stochastic models}
...
\begin{frame}{Application to non-normal series}
    ...
    \end{block}
    Box and Cox in 1964 suggest the following method of transforming and observed value $x$
    \begin{equation}
Y = \left\{
\begin{array}{c}
...
A further refinement is to introduce a second parameter by replacing $x$ with $x-\zeta$ say. Of special interest is the situation when $\lambda = 0$ in equation~\ref{e447}. This is the case in which the variable has a \emph{lognormal distribution}.
\end{frame}
 
\begin{frame}{Application to non-normal series}
    \begin{block}{Data generation using Markov model and lognormal distribution}
    \end{block}
    Consider the transformation $Y = \ln(X-\xi)$ where $X$ has \emph{mean} $\mu$ and \emph{variance} $\sigma_X^2$; $Y$ is normally distributed with \emph{mean} $\mu_Y$ and \emph{variance} $\sigma_Y^2$ and $\xi$ is a location parameter, which could be negative; also, the lag-l autocorrelation coefficients are $\rho_l$ and $\rho_{l,Y}$, respectively. 
...
\begin{equation}
    X_{t,\tau} = \mu_{\tau} + \sigma_{\tau} \sum_{i=1}^p \frac{\phi_{p,i} (X_{t-i,\tau-i} - \mu_{\tau-i})}{\sigma_{\tau-i}} + \sigma_{\tau} \eta_t
        \label{e453}
    \end{equation}
    in which $E[\eta_t] = E[\eta_t \eta_{t-k}] = 0$ for $k\neq0$ and $var[\eta_t] = E[\eta_t^2] = 1-R^2$; note that, if $\tau -i \leq 0$, $\tau-i$ should be replaced by $\tau-i +12$. The order $p$ and the autoregressive parameter $\phi_{p,i}$ are estimated from the standardised $\xi_t$ series. 

    Because $\mu_{\tau}$ and $\sigma_{\tau}$ have to be estimated from short historical samples, harmonic fitted statistics $\hat{\mu}_{\tau}$ and $\hat{\sigma}_{\tau}$ have been substituted in equation~\ref{e453} for estimating the autoregressive parameters. However, the resulting $\tilde{\xi_t}$ series and hence the autoregressive parameters may be significantly affected by sampling errors. This could happen when the \emph{sample statistics} $m_{\tau}$ and $s_{\tau}$ are used or when their harmonic fitted estimates $\hat{\mu}_{\tau}$ and $\hat{\sigma}_{\tau}$ are substituted  for the unknown parameters $\mu_{\tau}$ and $\sigma_{\tau}$. The advantage in the harmonic model is that it needs fewer parameters; also these are less affected by sample size. A further generalization of this model is possible by making $\phi_{p,i}$ seasonal but usually the differences between the seasonal  parameters are not significant. 
\end{frame}
...
This takes the form
\begin{equation}
    X_{t,\tau} = \frac{\mu_{\tau} + \sigma_{\tau} \rho_{\tau} ( X_{t-1, \tau-1} - \mu_{\tau-1})}{\sigma_{\tau-1}} + \sigma_{\tau} \eta_t
        \label{e454}
    \end{equation}
    in which
    \[
...
\]
is the coefficient of correlation between the flows in months $\tau$ and $\tau-1$. For the random component $E[\eta_t] = E[\eta_t \eta_{t-k}] = 0$ for $k\neq 0$. Also, recall that $var[\eta_t] = 1-\rho_{\tau}^2$.
